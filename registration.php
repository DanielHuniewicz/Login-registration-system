<?php
	...
	if(isset($_POST['mail']))
	{
		...
		$result_mail = $myDb->query("SELECT id FROM users WHERE mail='$mail'");

		$result_username = $myDb->query("SELECT id FROM users WHERE name='$user_name'");
...
*/
		...
		{
			if($myDb->query("INSERT INTO users VALUES ('$user_name','$password_hash','$mail')"))
			{
